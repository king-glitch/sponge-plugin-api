...
public class CommandService {
    ...
    public <T extends CommandExecutor> void register(T executor, Object plugin) throws AnnotatedCommandException {
        SpongeCommand command = buildCommandSpec(executor);
     ...
     * @throws AnnotatedCommandException the annotated command exception
     */
    public <T extends CommandExecutor> SpongeCommand buildCommandSpec(T command) throws AnnotatedCommandException {
        ...
        List<SpongeCommand> children = new ArrayList<>();
        // set children
        ...
        SpongeCommand com = new SpongeCommand(commandSpec, children, aliases);
    ...
    private CommandExecutor createHelpCommand(SpongeCommand command, ICommandHelpText annotation) {
        ...
        Map<SpongeCommand, Text> helpText = new LinkedHashMap<>();
        helpText.put(command, getHelpForBase(command, annotation));

        for (SpongeCommand child : command.getChildren()) {
    ...
    private Text getHelpForBase(SpongeCommand command, ICommandHelpText annotation) {
        return getHelpFor(command, annotation, command.getAliases()[0], "");
    }

    private Text getHelpForSub(SpongeCommand command, ICommandHelpText annotation, String base) {
        return getHelpFor(command, annotation, base + " ", command.getAliases()[0]);
    }

    private Text getHelpFor(SpongeCommand command, ICommandHelpText annotation, String base, String alias) {
